        ...
        <form action="{{ route('postalservice.update', $record->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select 
                    ...
                    id="receiver_city_id" 
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('receiver_city_id') border-red-500 @enderror" 
                    required>
                    <option value="">Choose a city</option>
                    @foreach($cities as $city)
                        <option value="{{ $city->id }}" {{ old('receiver_city_id', $record->receiver_city_id) == $city->id ? 'selected' : '' }}>
                            {{ ucfirst($city->name) }}
                        </option>
                    @endforeach
                </select>
