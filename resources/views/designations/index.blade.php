<x-app-layout>
                    ...
                    <form action="{{route('designations.index')}}" method="GET" class="mt-3">
                        <div class="flex flex-row flex-wrap items-center">
                            ...
                            <select name="search_type" id="search_type" class= "w-48 mt-1 border-gray-600 focus:ring-indigo-500  rounded-md shadow-sm" autofocus autocomplete="search_type">
                                <option value="designation" {{ old('search_type') == 'designation' ? 'selected' : '' }}>Designation</option>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach ($designations as $designation)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-6 py-4 text-sm text-gray-800 text-center">{{ $designation->id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ $designation->designation }}</td>
                                    <td>
                                        <button onclick="openEditModal({{ $designation->id }}, '{{ $designation->designation }}')" 
    ...
    <script>
        function openCreateModal() {
            document.getElementById("modalTitle").innerText = "Create New Designation";

            const form = document.getElementById("modalForm");
            form.action = "{{ route('designations.store') }}"; // store route
            ...
            document.getElementById("formFields").innerHTML = `
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="designation" id="designation" class="w-full border rounded p-2 mt-1">
                </div>
            `;
