<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto p-6 bg-white shadow-md rounded mt-10">
        ...
        @endif
        <form method="POST" action="{{ route('noc-other-district.update', $letter->Letter_ID) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <select name="letter_type" id="" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" onChange="toggleReferencedLetter(this.value)">
                        <option value="self" {{ old('letter_type', $letter->letter_type) == 'self' ? 'selected' : '' }}>Self</option>
                        <option value="official" {{ old('letter_type', $letter->letter_type) == 'official' ? 'selected' : '' }}>Official</option>
                    </select>
                </div>
                <div></div>
                ...
                <div id="referenced_letter_section" class="hidden">
                    ...
                    <input type="date" name="referenced_letter_date"
                           ...
                           value="{{ old('referenced_letter_date', optional($letter)->referenced_letter_date ? date('Y-m-d', strtotime($letter->referenced_letter_date)) : '') }}">
                </div>
                ...
                <div id="referenced_letter_no_section" class="hidden">
                    ...
                    <input type="text" name="referenced_letter_no"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                           value="{{ old('referenced_letter_no', $letter->referenced_letter_no) }}">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">
                        Letter Date
                    ...
                    <input type="date" name="Letter_Date"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                           value="{{ old('Letter_Date', optional($letter)->Letter_Date ? date('Y-m-d', strtotime($letter->Letter_Date)) : '') }}">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="NOC_Issued_To"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                           value="{{ old('NOC_Issued_To', $letter->NOC_Issued_To) }}">
                </div>
